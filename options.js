...
async function render() {
  ...
  $("#bindings").replaceChildren(...Object.entries(bindings).flatMap(([domain, ids]) => ids.map(id => {
    const div = document.createElement("div"); div.className = "row";
    const text = document.createElement("span"); text.textContent = `${domain} → ${byId[id]?.issuer || byId[id]?.name || "удалённый код"}`;
    const del = document.createElement("button"); del.textContent = "Удалить"; del.onclick = async () => { bindings[domain] = bindings[domain].filter(x => x !== id); if (!bindings[domain].length) delete bindings[domain]; await chrome.storage.local.set({ bindings }); render(); };
...
$("#bind").onclick = async () => {
  ...
  bindings[domain] = [...new Set([...(bindings[domain] || []), id])];
